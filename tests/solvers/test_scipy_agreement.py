"""`ScipySolver` runs under the `Solver` template method, and `Bisection` agrees with `scipy.optimize.bisect` there."""
...
def _catalog_cubic(x: float) -> float:
    return calibrated_cubic().build_x_fun(1.0)(x)


@pytest.mark.parametrize(
    "f, a, b", [(cube_minus_two, 0.0, 2.0), (lambda x: 0.3 - x, 0.0, 1.0), (_catalog_cubic, -2.0, 2.0)]
)
...
def test_bisection_agrees_with_scipy_bisect(f, a, b, xtol):
    ...
    # SciPy evaluates three more times: the two endpoint re-evaluations (see ScipySolver), plus one extra
    # halving because it stops at width < xtol where Interval.is_converged stops at 2 * xtol.
    assert scipy_result.n_fevals == ours.n_fevals + 3


def test_scipy_solver_reports_no_iterations_and_no_flops():
    """SciPy's loop runs on plain floats outside CountedFloat counting, so only evaluations are measured."""
    # --- act --------------------------
    scipy_result = ScipySolver(optimize.bisect).solve(cube_minus_two, 0.0, 2.0, xtol=1e-6, max_fevals=200)
...
def test_scipy_solver_is_subject_to_the_budget():
    """``max_fevals`` ends a SciPy solve the same way it ends ours."""
    # --- act --------------------------
    scipy_result = ScipySolver(optimize.bisect).solve(cube_minus_two, 0.0, 2.0, xtol=1e-12, max_fevals=5)
