...
def test_user_subcategory_under_99_registers_with_its_formulas():
    # --- arrange / act ----------------
    category = category_cls((99, 1))
    formula = minimal_formula_cls((99, 1, 1))
...
@pytest.mark.usefixtures("isolated_registry")
@pytest.mark.parametrize("define_second", [category_cls, minimal_formula_cls])
...
def test_builtin_only_flag_below_the_top_level_is_rejected():
    with pytest.raises(ValueError, match="only a top-level category declares it"):
        category_cls((99, 1), is_builtin_only=True)


# ==================================================================================================
#  taxonomy checks, run on the first read
# ==================================================================================================
@pytest.mark.usefixtures("isolated_registry")
def test_formula_may_register_before_its_category():
    """A category package imports its formula modules at the top, so formulas register first."""
    # --- arrange ----------------------
    formula = minimal_formula_cls((99, 1, 1))
...
@pytest.mark.usefixtures("isolated_registry")
@pytest.mark.parametrize("define_orphan", [category_cls, minimal_formula_cls])
def test_node_without_parent_category_fails_on_read(define_orphan):
    # --- arrange ----------------------
    define_orphan((99, 7, 1))  # no category (99, 7)
...
@pytest.mark.usefixtures("isolated_registry")
@pytest.mark.parametrize("define_node, number", [(category_cls, (2, 50)), (minimal_formula_cls, (2, 1, 50))])
