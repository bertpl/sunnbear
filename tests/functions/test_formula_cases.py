...
def _positional(formula, params):
    assert set(params) == set(formula.param_names), (
        f"{type(formula).__name__}: case params {sorted(params)} != param_names {sorted(formula.param_names)}"
    )
    return tuple(params[name] for name in formula.param_names)
...
def test_formula_test_case(formula, case):
    ...
    match case:
        ...
        case InvalidTestCase(params=params):
            assert not formula.is_param_tuple_valid(*_positional(formula, params))
        case BracketTestCase(params=params, expected=(lo, hi), rtol=rtol, atol=atol):
            got_lo, got_hi = formula.bracket(*_positional(formula, params))
