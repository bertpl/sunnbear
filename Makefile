...
help:
	...
	@echo '  coverage                       Run unit tests with coverage; HTML report in htmlcov/.'
...
release:
	...
	# Run the preconditions first: they take seconds (plus any wait for main CI on HEAD to finish),
	# so a bad changelog or a failed CI run aborts before the test suite runs for minutes.
