...
class TaxonomyNode:
    ...
    def _validate_number(cls, min_length: int) -> None:
        ...
        Raises:
            ...
            ValueError: If `number` is shorter than `min_length`, or has an element below 1.
        """
        ...
        if len(number) < min_length:
            raise ValueError(f"{cls.__name__}.number must have at least {min_length} element(s) (got {number!r}).")
